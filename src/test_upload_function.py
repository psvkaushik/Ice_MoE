...
from loaders_fast import AI4ArcticChallengeTestDataset, get_variable_options
from functions import slide_inference, batched_slide_inference



def test(mode: str, net: torch.nn.modules, checkpoint: str, device: str, cfg, test_list, gt_list=None, test_name='test'):
    """_summary_
    ...
    experiment_name = 'test1' #osp.splitext(osp.basename(cfg.work_dir))[0]
    ...
    if mode == 'test':
        ...
        train_options['test_list_gt'] = gt_list
        ...
        dataset = AI4ArcticChallengeTestDataset(
            options=train_options, files=train_options['test_list'], files_gt=train_options['test_list_gt'], mode='test')
    ...
    for inf_x, inf_y, cfv_masks, tfv_mask, scene_name, original_size in tqdm(iterable=asid_loader,
        ...
        inf_x = inf_x.to(device, dtype=torch.bfloat16, non_blocking=True)
        ...
        for j in range(0, 2):
            ax = axs[j]
            img = torch.squeeze(inf_x, dim=0).cpu().float().numpy()[j]
            if j == 0:
