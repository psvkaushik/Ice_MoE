"""
Helper functions copied from AI4ArcticSeaIceChallenge/functions.py,MMSeaIce, used for plotting and metric calculation.
https://github.com/astokholm/AI4ArcticSeaIceChallenge
https://github.com/echonax07/MMSeaIce/blob/main/functions.py
...
# from unet_attn import UNet as UNet_attn
    ...
    for chart in charts:
        if true[chart].ndim == 1 and pred[chart].ndim == 1:
            if chart == 'SIC':
                ...
                scores[chart] = np.round(metrics[chart]['func'](true=true[chart], pred=pred[chart], chart=chart) * 100, 3)
        else:
...
def f1_metric(true, pred, chart=None):
    """
...
def tile_inference(img, net, options):
    ...
    SIC_c  = 1
    # print(SIC_c)
    SOD_c  = options['n_classes']['SOD']
    FLOE_c = options['n_classes']['FLOE']
    
    preds_SIC  = img.new_zeros((B, SIC_c, H, W))
    preds_SOD  = img.new_zeros((B, SOD_c, H, W))
    preds_FLOE = img.new_zeros((B, FLOE_c, H, W))
    ...
    for y in range(0, H, patch):
        for x in range(0, W, patch):
            ...
            # pad only if needed
            if h_crop < patch or w_crop < patch:
                crop = torch.nn.functional.pad(
                    crop,
                    (0, patch - w_crop, 0, patch - h_crop)
                )

            out = net(crop)

            # remove padded predictions
            sic  = out['SIC'][:, :, :h_crop, :w_crop]
            sod  = out['SOD'][:, :, :h_crop, :w_crop]
            floe = out['FLOE'][:, :, :h_crop, :w_crop]

            # write directly (no averaging)
            preds_SIC[:, :, y:y2, x:x2]  = sic
            preds_SOD[:, :, y:y2, x:x2]  = sod
            preds_FLOE[:, :, y:y2, x:x2] = floe
...
def make_patches(scene, patch_size=256):
    ...
    if pad_h > 0 or pad_w > 0:
        ...
        padded = torch.zeros(C, padded_H, padded_W)
...
def patch_inference(img, net, options):
    patches = make_patches(img, 256)
    output = {
        "SIC": torch.zeros_like(img[:, :options['n_classes']['SIC'], :, :]),
        "SOD": torch.zeros_like(img[:, :options['n_classes']['SOD'], :, :]),
        "FLOE": torch.zeros_like(img[:, :options['n_classes']['FLOE'], :, :])
    }
    for patch in patches:
        out = net(patch.unsqueeze(0))
        output['SIC'] += out['SIC']
        output['SOD'] += out['SOD']
        output['FLOE'] += out['FLOE']
...
def create_train_validation_and_test_scene_list(train_options):
    ...
    # Test ----------
    with open(train_options['path_to_env'] + train_options['test_x_json']) as file:
        train_options['test_list_x'] = json.loads(file.read())
    with open(train_options['path_to_env'] + train_options['test_gt_json']) as file:
        train_options['test_list_gt'] = json.loads(file.read())
...
def get_loss(loss, chart=None, **kwargs):
    ...
    if loss == 'DiceLoss':
        ...
        from losses import FLOELoss
        loss = FLOELoss()
    elif loss == 'OrdinalRegressionLoss':
...
def get_model(train_options, device):
    ...
    if train_options['model_selection'] == 'h_unet':
    ...
    elif train_options['model_selection'] in ['moe_full']:
        from model import UNet_MTL_MMoE
        net = UNet_MTL_MMoE(options=train_options).to(device)
    else:
