...
https://github.com/echonax07/MMSeaIce
...
def parse_args():
    ...
    parser.add_argument('--work-dir', help='the dir to save logs and models', default='/mnt/beegfs/spillal2/STAC/unet/logs/')
...
def train(cfg, train_options, net, device, dataloader_train, dataloader_val, optimizer, scheduler, start_epoch=0):
    ...
    for epoch in tqdm(iterable=range(start_epoch, train_options['epochs'])):
        ...
        # net.to(dtype=torch.bfloat16)  # Set network to bfloat16 to save memory.
        ...
        for i, (batch_x, batch_y) in enumerate(tqdm(iterable=dataloader_train, total=train_options['epoch_len'],
            ...
            # - Transfer to device.
            batch_x = batch_x.to(device, non_blocking=True, dtype=torch.bfloat16)
            ...
            # train_loss_batch += net.load_balancing_loss()
        ...
        for i, (inf_x, inf_y, cfv_masks, tfv_mask, name, original_size) in enumerate(tqdm(iterable=dataloader_val,
            ...
            with torch.no_grad(), torch.amp.autocast('cuda', dtype=torch.bfloat16):
                inf_x = inf_x.to(device, non_blocking=True, dtype=torch.bfloat16)
        ...
        if combined_score > best_combined_score:
            ...
            wandb.run.summary[f"While training/Best Combined Score"] = combined_score
...
def main():
    ...
    if not train_options['cross_val_run']:
        wandb.init(name='dummy_exp', project=args.wandb_project,
                   entity="enter_yours", config=train_options, id=id, resume="allow")
    else:
        wandb.init(name='dummy_exp', group=osp.splitext(osp.basename(args.config))[0], project=args.wandb_project,
                   entity="enter_yours", config=train_options, id=id, resume="allow")
    ...
    test('val', net, checkpoint_path, device, cfg.deepcopy(), train_options['validate_list'], test_name='Cross Validation')
    ...
    test('test', net, checkpoint_path, device, cfg.deepcopy(), train_options['test_list_x'], gt_list=train_options['test_list_gt'], test_name='Test')
