...
class AI4ArcticChallengeDataset(Dataset):
    ...
    def __init__(self, options, files, do_transform=False):
        ...
        if self.downsample:
            print("Downsampling data and loading on memory. This may take a while...")
            self.scenes = []
            self.amsrs = []
            self.aux = []
...
class AI4ArcticChallengeTestDataset(Dataset):
    ...
    def __init__(self, options, files, files_gt=None, mode='test'):
        self.options = options
        self.files =sorted(files)
        self.files_gt = files_gt

        if files_gt is not None:
            self.files_gt = sorted(files_gt)
            # for i in range(len(self.files)):
            #     if self.files_gt[i].split('/')[-1].split('.')[0] != self.files[i].split('/')[-1].split('.')[0]:
            #         raise ValueError(f"Files in files_gt must match files in files. Mismatch found in {self.files_gt[i]} and {self.files[i]}")
    ...
    def prep_scene(self, scene, gt=None):
        """
        ...
        if self.mode != 'test_no_gt':
            # gt = xr.open_dataset
            if self.mode == 'test':
                y_charts = torch.from_numpy(gt[self.options['charts']].isel().to_array().values).unsqueeze(0)
            else:
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'test' or  self.mode == 'test_no_gt':
            ...
            gt = xr.open_dataset(os.path.join(
                self.options['path_to_test_data'], self.files_gt[idx]), engine='h5netcdf')
            x, y = self.prep_scene(scene, gt=gt)
            name = self.files[idx]
